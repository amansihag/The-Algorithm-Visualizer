...
function partition(divs, height, l, r){
    ...
    while(i<=j)
    {
        ...
        if(i < j)
        {
            ...
            height[i] = height[j];
            height[j] = temp;
            div_update(divs[2*(i)+1],height[i],'turquoise');
            div_update(divs[2*(j)+1],height[j],'turquoise');
            div_update(divs[2*(i)+1],height[i],'#808080');
            div_update(divs[2*(j)+1],height[j],'#808080');
        }
    }
    if(j!=p)
    {
        var temp = height[p];
            height[p] = height[j];
            height[j] = temp;
            div_update(divs[2*(p)+1],height[p],'turquoise');
            div_update(divs[2*(j)+1],height[j],'turquoise');
            div_update(divs[2*(p)+1],height[p],'#808080');
            div_update(divs[2*(j)+1],height[j],'#808080');
    }
    return j;
}
